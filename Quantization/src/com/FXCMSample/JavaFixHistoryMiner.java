  ...
  implements IGenericMessageListener, IStatusMessageListener
{
  ...
  public String sendRequest(ITransportable request)
  {
    try
    {
      synchronized ( requestSync ) {
        ...
        requestComplete = false;
        output.println("sendRequest(ITransportable request)");
  ...
  {
      output.println("messageArrived(ISessionStatus status)");
  ...
  @Override public void messageArrived(ITransportable message)
  {
    ...
    try
    {
        output.println("messageArrived(ITransportable message)"+message.getClass().getName());
  ...
  public void messageArrived(CollateralReport cr)
  {
    synchronized ( requestSync ) {
      ...
      if(currentRequest.equals(cr.getRequestID()) && !accounts.contains(cr))
      {
        ...
        requestSync.notify();
        output.println("messageArrived(CollateralReport cr)");
      }
    }
  }
  ...
  public void messageArrived(TradingSessionStatus tss)
  {
    synchronized ( requestSync ) {
      ...
      if(currentRequest.equals(tss.getRequestID()))
      {
        ...
        requestSync.notify();
        output.println("messageArrived(TradingSessionStatus tss)");
        ...
        try
        {
          ...
          // send the request
          output.println("sendRequest(mdr);");
          sendRequest(mdr);
        }
        catch(Exception e) { 
          Logger.getLogger(JavaFixHistoryMiner.class.getName()).log(Level.SEVERE, null, e);
  ...
  public void messageArrived(MarketDataRequestReject mdrr)
  {
    synchronized ( requestSync ) {
      ...
      requestComplete = true;
      requestSync.notify();
      output.println("messageArrived(MarketDataRequestReject mdrr)");
    }
  }
    ...
    synchronized ( requestSync ) { // currently we don't have a locking order issue with requestSync and historicalRates
      ...
      try
      {
        if(mds.getRequestID() != null && mds.getRequestID().equals(currentRequest))
        {
          ...
          requestSync.notify();
          output.println("messageArrived(MarketDataSnapshot mds)");
        }
      }
      catch (Exception e)
      {
  ...
  public void displayHistory()
  {
    ...
    output.println("Date\t   Time\t\tOAsk\tOBid\tCAsk\tCBid\tHAsk\tHBid\tLAsk\tLBid");
  ...
  public void displayHistory(MarketDataSnapshot candleData)
  {
    // define a format for the dates
    SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy HH:mm:ss z");
    // make the date formatter above convert from GMT to EST
    sdf.setTimeZone(TimeZone.getTimeZone("PRC"));
  	...
  	output.println(
    sdf.format(candleData.getDate().toDate()) + "\t" +    // the date and time formatted and converted to EST
    	    
	sdf.format(candleData.getOpenTimestamp().toDate()) + "\t" +
	sdf.format(candleData.getCloseTimestamp().toDate()) + "\t" +
